# Agave Platform Science API
#
# Power your digital lab and reduce the time from theory to discovery using the Agave Science-as-a-Service API Platform. Agave provides hosted services that allow researchers to manage data, conduct experiments, and publish and share results from anywhere at any time.
#
# Agave Platform version: 2.2.14
# 
# Generated by: https://github.com/swagger-api/swagger-codegen.git


#' MonitorCheckResultType Class
#'
#' 
#'
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
MonitorCheckResultType <- R6::R6Class(
  'MonitorCheckResultType',
  public = list(
    initialize = function(){
    },
    asJSON = function() {
      self$toJSON()
    },
    toJSON = function() {
      MonitorCheckResultTypeObject <- list()

      MonitorCheckResultTypeObject
    },
    fromJSON = function(MonitorCheckResultTypeObject) {
      if (is.character(MonitorCheckResultTypeObject)) {
        MonitorCheckResultTypeObject <- jsonlite::fromJSON(MonitorCheckResultTypeJson)
      }

      if ("result" %in% names(MonitorCheckResultTypeObject)) {
        MonitorCheckResultTypeObject <- MonitorCheckResultTypeObject$result
      }

    },
    toJSONString = function() {
       sprintf(
        '{
        }',
      )
    },
    fromJSONString = function(MonitorCheckResultTypeJson) {
      MonitorCheckResultTypeObject <- jsonlite::fromJSON(MonitorCheckResultTypeJson)
      self::fromJSON(MonitorCheckResultTypeObject)

    }
  )
)
