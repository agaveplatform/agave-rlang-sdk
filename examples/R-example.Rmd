...
```{r}
ag$apps$listApps(search=c("id.eq" = app$id))[[1]]
```
...
```{r}
queryParams <- list("name.like" = "opensees")
...
queryParams
paste0(c("http://example.com", paste0(lapply(names(queryParams), function(x) paste0(c(x,queryParams[x]), collapse = '=')), collapse = '&')), collapse = '?')
logger.trace()
```
...
apps = ag$apps$list_apps(search=list("id.like" = "*docker*"), limit=2)
#httr::content(apps$response, 'text', encode="UTF-8")
length(apps)
```
...
```{R}
ag$apps$list(search=c("name.like" = "*docker*", "revision.gte" = 3))
```
